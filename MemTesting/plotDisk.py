...
import os
import re
from plotMemory import getAlgorithm, match_name

def main():
    # Step 1: Read the data from the files
    n = "*"
    p = "10"
    file_pattern = "newResults/*.py_n_"+n+"_p_"+p+"/*/diskResults.txt"  # Adjust the path and pattern as needed
    ...
    plt.title('Disk space Used by Process Over Time - ' + getAlgorithm(p))
    plt.xlabel('Time (minutes)')
    plt.ylabel('Memory Used (MB)')
    # plt.legend(title='Source File')
    plt.grid(True)
    plt.tight_layout()
    plt.savefig('disk_usage_plot_' + getAlgorithm(p) + '.png')  # Save the plot to a file
    plt.show()
