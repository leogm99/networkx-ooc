...
# La dif con la clase de EdgesDict es que esta maneja distinto los None. Tratar de refactorizar.
class BiTupleDict(OutOfCoreDict):
    def __init__(self, key_primitive_type: PrimitiveType = PrimitiveType.TUPLE, value_primitive_type: PrimitiveType = PrimitiveType.INTEGER):
        if key_primitive_type != PrimitiveType.TUPLE and value_primitive_type != PrimitiveType.TUPLE:
            raise ValueError("Key or value type must be TUPLE")
        super().__init__()
        self._key_format = key_primitive_type.value
        self._value_format = value_primitive_type.value
    ...
    def __serialize_key(self, key):
        if self._key_format == PrimitiveType.TUPLE:
            return struct.pack(self._key_format, *key)

        return struct.pack(self._key_format, key)

    def __serialize_value(self, value):
        if self._value_format == PrimitiveType.TUPLE:
            return struct.pack(self._value_format, *value)
