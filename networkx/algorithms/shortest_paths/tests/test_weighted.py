...
class TestWeightedPath(WeightedTestBase):
    ...
    def test_dijkstra_pred_distance_multigraph(self):
        G = nx.MultiGraph()
        G.add_edge(1, 2, key="short", foo=5, weight=100)
        G.add_edge(1, 2, key="long", bar=1, weight=110)
        p, d = nx.dijkstra_predecessor_and_distance(G, 1)
        assert p == {1: [], 2: [1]}
        assert d == {1: 0, 2: 100}
...
class TestJohnsonAlgorithm(WeightedTestBase):
    def test_single_node_graph(self):
        G = nx.DiGraph()
        G.add_node(0)
        assert dict(nx.johnson(G)) == {0: {0: [0]}}
    ...
    def test_negative_cycle(self):
        ...
        johnson_result = nx.johnson(G)
        try:
            for _ in johnson_result:
                assert False
    ...
    def test_unweighted_graph(self):
        ...
        assert dict(nx.johnson(G)) == dict(nx.johnson(H))
