...
class TestBipartiteCentrality:
    ...
    def setup_class(cls):
        ...
        cls.LazyP4 = LazyGraph()
        for e in cls.P4.edges:
            cls.LazyP4.add_edge(*e)

        cls.K3 = nx.complete_bipartite_graph(3, 3)
        cls.LazyK3 = LazyGraph()
        for e in cls.K3.edges:
            cls.LazyK3.add_edge(*e)

        cls.C4 = nx.cycle_graph(4)
        cls.LazyC4 = LazyGraph()
        for e in cls.C4.edges:
            cls.LazyC4.add_edge(*e)
    ...
    def test_degree_centrality(self):
        d = bipartite.degree_centrality(self.LazyP4, [1, 3])
        answer = {0: 0.5, 1: 1.0, 2: 1.0, 3: 0.5}
        assert d == answer
        d = bipartite.degree_centrality(self.LazyK3, [0, 1, 2])
        answer = {0: 1.0, 1: 1.0, 2: 1.0, 3: 1.0, 4: 1.0, 5: 1.0}
        assert d == answer
        d = bipartite.degree_centrality(self.LazyC4, [0, 2])
        answer = {0: 1.0, 1: 1.0, 2: 1.0, 3: 1.0}
        assert d == answer

    def test_betweenness_centrality(self):
        c = bipartite.betweenness_centrality(self.LazyP4, [1, 3])
        answer = {0: 0.0, 1: 1.0, 2: 1.0, 3: 0.0}
        assert c == answer
        c = bipartite.betweenness_centrality(self.LazyK3, [0, 1, 2])
        answer = {0: 0.125, 1: 0.125, 2: 0.125, 3: 0.125, 4: 0.125, 5: 0.125}
        assert c == answer
        c = bipartite.betweenness_centrality(self.LazyC4, [0, 2])
        answer = {0: 0.25, 1: 0.25, 2: 0.25, 3: 0.25}
        assert c == answer

    def test_closeness_centrality(self):
        c = bipartite.closeness_centrality(self.LazyP4, [1, 3])
        answer = {0: 2.0 / 3, 1: 1.0, 2: 1.0, 3: 2.0 / 3}
        # assert c == answer
        for key in c:
            assert round(c[key], 6) == round(answer[key], 6)
        c = bipartite.closeness_centrality(self.LazyK3, [0, 1, 2])
        answer = {0: 1.0, 1: 1.0, 2: 1.0, 3: 1.0, 4: 1.0, 5: 1.0}
        assert c == answer
        c = bipartite.closeness_centrality(self.LazyC4, [0, 2])
        answer = {0: 1.0, 1: 1.0, 2: 1.0, 3: 1.0}
        assert c == answer
        G = nx.Graph()
        G.add_node(0)
    ...
    def test_bipartite_closeness_centrality_unconnected(self):
        ...
        for e in G.edges:
            LazyG.add_edge(*e)
        LazyG.add_edge(6, 7)
        c = bipartite.closeness_centrality(LazyG, [0, 2, 4, 6], normalized=False)
        answer = {
            0: 10.0 / 7,
            2: 10.0 / 7,
            4: 10.0 / 7,
